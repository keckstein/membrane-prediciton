...
val_gen = parallel_data_generator(
    ...
    spacing=(64, 64, 64),
    area_size=[(raw_3[0:64, :, 0:357]).shape],
    target_shape=(64, 64, 64),
    gt_target_shape=(64, 64, 64),
    stop_after_epoch=False,
...
network = PiledUnet(n_nets=2,
                    ...
                    filter_sizes_down=(
                        ((8, 16), (16, 32), (32, 64)),
                        ((8, 16), (16, 32), (32, 64)) #((8, 16), (16, 32), (32, 64))
                    ),
                    filter_sizes_bottleneck=(
                        (64, 128),
                        (64, 128),
...
writer = SummaryWriter(
    '/g/schwab/eckstein/scripts/tensorboard/piled_unet_23_run1_new_gt_brightness_32' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
loss = CombinedLosses(losses=(
    ...
    WeightMatrixWeightedBCE(((0.4, 0.6),), weigh_with_matrix_sum=False)), #WeightMatrixWeightedBCE(((0.5, 0.5),), weigh_with_matrix_sum=False),,
    y_pred_channels=(np.s_[:1], np.s_[1:2]), #np.s_[2:3]
    y_true_channels=(np.s_[:], np.s_[:]), # np.s_[:]
    weigh_losses=np.array([0.4, 0.6]) # 0.5
)
            ...
            for x_val, y_val, val_epoch, val_n, val_loe in val_gen:
                ...
                if val_loe:
                    ...
                    if best_val_loss is None or val_loss < best_val_loss:
                        ...
                        torch.save(network.state_dict(), f'/scratch/eckstein/models/piled_unet_23_run1_new_gt_brightness_32/result{epoch:04d}.h5')
                    break
